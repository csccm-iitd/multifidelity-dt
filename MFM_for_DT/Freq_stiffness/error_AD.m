...
for i=1:4
    if i==1
s=load('freqD_stiffness_mAD_3')
    elseif i==2
s=load('freqD_stiffness_mAD_4')
    elseif i==3
s=load('freqD_stiffness_mAD_5')
    elseif i==4
s=load('freqD_stiffness_mAD_10') 
    end        
...
Error_m=s.Identified_stiffness_array+1-Kp_MF;
sq_m=Error_m.^2;              
MSE_m=sum(sq_m)/no;        %sample_variance
RMSE_m=sqrt(MSE_m)

Error_h=s.Identified_stiffness_array+1-Kp_HF;
sq_h=Error_h.^2;
MSE_h=sum(sq_h)/no;
RMSE_h=sqrt(MSE_h)

% calculation of coefficient of determination
mean_original_data=sum(s.Identified_stiffness_array+1)/no; %mean of actual system
deviation=s.Identified_stiffness_array+1-ones(no,1)*mean_original_data;
